...
public class FieldType extends AbstractType {
    ...
    public FieldType(Node node, List<String> types) {
        super(node);
        this.types = types;
    }

    public String getType() {
        return getElement().hasAttribute("type") ? (types.contains(getElement().getAttribute("type")) ? getElement().getAttribute("type") : "String") : (isClass() ? getClassName() : "String");
    }

    public boolean isList() {
        return getElement().hasAttribute("maxOccurs");
    }
    ...
    public String toString() {
        return getFieldName() + " [type: " + (isList() ? "List{" : "") + getType() + (isList() ? "}" : "") + "]";
    }
